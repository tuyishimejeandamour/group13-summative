...
class ImageProcessor:
    def __init__(self, base_dir=None, target_size=(224, 224), users=None):
        ...
        self.users = users if users else self._auto_detect_users()
    ...
    def process_single_image(self, image_path, user, expression):
        ...
        augmentation_types = [
            'original', 'rotation_15', 'rotation_neg15', 'horizontal_flip',
            'brightness_increase', 'brightness_decrease', 'grayscale', 'blur'
        ]
    ...
    def process_all_images(self):
        ...
        for user in self.users:
            ...
            for img_file in image_files:
                ...
                if 'neutral' in filename_lower or 'normal' in filename_lower:
                    expression = 'neutral'
                elif 'smil' in filename_lower:
                    expression = 'smiling'
                elif 'surpris' in filename_lower:
    ...
    def save_features_to_csv(self, df_features, output_path=None):
        if output_path is None:
            face_recognition_dir = Path(__file__).resolve().parent.parent
            output_path = face_recognition_dir / 'data' / 'features' / 'image_features.csv'
        else:
    ...
    def display_sample_images(self, save_output=True):
        ...
        if not self.users:
            print("ERROR: No users found. Cannot display images.")
            print(f"Please check that user directories exist in: {self.base_dir}")
            return 0, 0
        
        expressions = ['neutral', 'smiling', 'surprised']
        expression_variations = {
            'neutral': ['neutral', 'normal'],
            'smiling': ['smiling', 'smilling', 'similing', 'smile'],
        ...
        for row_idx, user in enumerate(self.users):
            ...
            for col_idx, expression in enumerate(expressions):
                ...
                if os.path.exists(user_dir):
                    possible_names = expression_variations.get(expression, [expression])
                    for expr_name in possible_names:
                        for ext in extensions:
                            test_path = os.path.join(user_dir, f"{expr_name}{ext}")
                ...
                if img_path and os.path.exists(img_path):
                    ...
                    if img is not None:
                        ...
                        title = f'{user}\n{expression.capitalize()}' if col_idx == 0 else expression.capitalize()
        ...
        if save_output:
            face_recognition_dir = Path(__file__).resolve().parent.parent
            output_path = face_recognition_dir / 'reports' / 'sample_images.png'
            output_path.parent.mkdir(parents=True, exist_ok=True)
